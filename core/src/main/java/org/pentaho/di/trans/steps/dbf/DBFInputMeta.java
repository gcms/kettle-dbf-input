...
@Step( id = "DBFInput",
      image = "XBI.svg",
      i18nPackageName="org.pentaho.di.trans.steps.dbf",
      name="DBFInputDialog.Dialog.Title",
      description = "DBFInputDialog.Dialog.Title",
      categoryDescription="i18n:org.pentaho.di.trans.step:BaseStep.Category.Input" )
public class DBFInputMeta extends BaseStepMeta implements StepMetaInterface {
  ...
  @Override
  public Object clone() {
    DBFInputMeta retval = (DBFInputMeta) super.clone();
    return retval;
  }
  ...
  private void readData( Node stepnode ) throws KettleXMLException {
    try {
      ...
      throw new KettleXMLException( BaseMessages.getString(
        PKG, "DBFInputMeta.Exception.UnableToReadStepInformationFromXML" ), e );
    }
  }
  ...
  public RowMetaInterface getOutputFields( FileInputList files, String name ) throws KettleStepException {
    ...
    DBF xbi = null;
    try {
      xbi = DBFFactory.createDBF( getLog(), files.getFile( 0 ), fileCompression );
      ...
      throw new KettleStepException( BaseMessages.getString(
        PKG, "DBFInputMeta.Exception.UnableToReadMetaDataFromDBFFile" ), ke );
    } finally {
      if ( xbi != null ) {
        xbi.close();
      }
  ...
  public void getFields( RowMetaInterface row, String name, RowMetaInterface[] info, StepMeta nextStep,
    ...
    if ( fileList.nrOfFiles() == 0 ) {
      throw new KettleStepException( BaseMessages
        .getString( PKG, "DBFInputMeta.Exception.NoFilesFoundToProcess" ) );
    }
  ...
  public void readRep( Repository rep, IMetaStore metaStore, ObjectId id_step, List<DatabaseMeta> databases ) throws KettleException {
    try {
      ...
      throw new KettleException( BaseMessages.getString(
        PKG, "DBFInputMeta.Exception.UnexpectedErrorReadingMetaDataFromRepository" ), e );
    }
  }
  ...
  public void saveRep( Repository rep, IMetaStore metaStore, ObjectId id_transformation, ObjectId id_step ) throws KettleException {
    try {
      ...
      throw new KettleException( BaseMessages.getString(
        PKG, "DBFInputMeta.Exception.UnableToSaveMetaDataToRepository" )
        + id_step, e );
    }
  }
    ...
    if ( dbfFileName == null ) {
      if ( isAcceptingFilenames() ) {
        if ( Utils.isEmpty( getAcceptingStepName() ) ) {
          cr =
            new CheckResult( CheckResult.TYPE_RESULT_ERROR, BaseMessages.getString(
              PKG, "DBFInput.Log.Error.InvalidAcceptingStepName" ), stepMeta );
          remarks.add( cr );
        }
        ...
        if ( Utils.isEmpty( getAcceptingField() ) ) {
          cr =
            new CheckResult( CheckResult.TYPE_RESULT_ERROR, BaseMessages.getString(
              PKG, "DBFInput.Log.Error.InvalidAcceptingFieldName" ), stepMeta );
          remarks.add( cr );
        }
      } else {
        cr =
          new CheckResult( CheckResult.TYPE_RESULT_ERROR, BaseMessages.getString(
            PKG, "DBFInputMeta.Remark.PleaseSelectFileToUse" ), stepMeta );
        remarks.add( cr );
      }
    } else {
      cr =
        new CheckResult( CheckResult.TYPE_RESULT_OK, BaseMessages.getString(
          PKG, "DBFInputMeta.Remark.FileToUseIsSpecified" ), stepMeta );
      remarks.add( cr );

      DBF xbi = null;
      try {
        xbi = DBFFactory.createDBF( log, transMeta.environmentSubstitute( dbfFileName ), fileCompression );
        cr =
          new CheckResult( CheckResult.TYPE_RESULT_OK, BaseMessages.getString(
            PKG, "DBFInputMeta.Remark.FileExistsAndCanBeOpened" ), stepMeta );
        remarks.add( cr );
        ...
        cr =
          new CheckResult( CheckResult.TYPE_RESULT_OK, r.size()
            + BaseMessages.getString( PKG, "DBFInputMeta.Remark.OutputFieldsCouldBeDetermined" ), stepMeta );
        remarks.add( cr );
      } catch ( KettleException ke ) {
        cr =
          new CheckResult( CheckResult.TYPE_RESULT_ERROR, BaseMessages.getString(
            PKG, "DBFInputMeta.Remark.NoFieldsCouldBeFoundInFileBecauseOfError" )
  ...
  public StepInterface getStep( StepMeta stepMeta, StepDataInterface stepDataInterface, int cnr, TransMeta tr,
    Trans trans ) {
    return new DBFInput( stepMeta, stepDataInterface, cnr, tr, trans );
  }

  @Override
  public StepDataInterface getStepData() {
    return new DBFInputData();
  }
